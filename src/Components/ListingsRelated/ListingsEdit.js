                        ...
                        if (d < files.length) {
                            ...
                            fileReader.onload = function (event) {
                                ...
                                if(eventNameRef.current.value == ''){
                                    dataObj.JobTitle = singleVolunteer.JobTitle;
                                }
                                if(eventOrganization.current.value == ''){
                                    dataObj.SkillDescription = singleVolunteer.SkillDescription;
                                }
                                if(eventDescription.current.value == ''){
                                    dataObj.JobAvailability = singleVolunteer.JobAvailability;
                                }
                                if(volunteersNeeded.current.value == ''){
                                    dataObj.JobDescription = singleVolunteer.JobDescription;
                                }
                                if(eventDescription.current.value == ''){
                                    dataObj.Contact = singleVolunteer.Contact
                                }
                        
                                fetch(`${process.env.REACT_APP_FETCH_LINK}/updateJobs`, {
                                    method: "POST",
                                    cache: 'no-cache',
